...
public class PatientNameQueryHelperTest {
	...
	public void shouldReturnWhereClauseWithNameSearchConditionWhenWildCardParameterWithAnyString() throws Exception {
		...
		String whereClause = patientNameQueryHelper.appendToWhereClause("where clause");
		assertEquals("where clause and ( concat_ws(' ',coalesce(given_name), coalesce(middle_name), coalesce(family_name)) like  '%James%') or ( pa_name.value like '%James%')", whereClause);
	}
	...
	public void shouldReturnWhereClauseWithNameSearchConditionWhenWildCardParameterWithMultipleStrings() throws Exception {
		...
		String whereClause = patientNameQueryHelper.appendToWhereClause("where clause");
		assertEquals("where clause and ( concat_ws(' ',coalesce(given_name), coalesce(middle_name), coalesce(family_name)) like  '%James%' and  concat_ws(' ',coalesce(given_name), coalesce(middle_name), coalesce(family_name)) like  '%Bond%') or ( pa_name.value like '%James Bond%')", whereClause);
	}
	...
	public void shouldReturnWhereClauseWithNameSearchConditionWhenWildCardParameterWithSingleQuote() throws Exception {
		...
		String whereClause = patientNameQueryHelper.appendToWhereClause("where clause");
		assertEquals("where clause and ( concat_ws(' ',coalesce(given_name), coalesce(middle_name), coalesce(family_name)) like  '%James%' and  concat_ws(' ',coalesce(given_name), coalesce(middle_name), coalesce(family_name)) like  '%Bo''nd%') or ( pa_name.value like '%James Bo''nd%')", whereClause);
